...
- 所有业务参数必须平铺在 JSON body 顶层并和 `api_name` 同级；不要包在 `params` 对象里。统一调用脚本会自动注入 `skill_version`。
...
正确：首页请求，参数平铺。
```json
{"api_name":"/user/notebooks","count":20}
```

正确：下一页请求，`lastSort` 取上一页 `books` 最后一项的 `sort`。
```json
{"api_name":"/user/notebooks","count":20,"lastSort":1778312777}
```

错误：不要使用 `params` 包裹业务参数，否则后端收不到 `count` 和 `lastSort`。
```json
{"api_name":"/user/notebooks","params":{"count":20,"lastSort":1778312777}}
```

错误：不要使用 `offset`/`limit`，这些字段不是本接口分页参数。
```json
{"api_name":"/user/notebooks","offset":20,"limit":20}
```

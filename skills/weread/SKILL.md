---
name: weread
description: 微信读书（WeRead）助手，用于搜索图书、浏览书架及分组、查询阅读进度与统计，以及读取笔记、划线、书评和推荐。
metadata:
  version: "1.0.4"
---
...
| 阅读概况 | 汇总书架、最近阅读进度和笔记概况 | "看看我的阅读概况" | `profile.md` |
...
- Header：`Authorization: Bearer $WEREAD_TOKEN`
- `WEREAD_TOKEN` 由 WeRead connector 提供，格式 `wrk-xxxxxxxx`
- 若未设置，运行 `npx --yes --package="${CLI_PKG_URL}" okou connector check --env-name WEREAD_TOKEN` 诊断连接
...
- **Body**：JSON，`api_name` 指定接口，其余为接口参数；发往服务端的请求必须带具体的 `skill_version`
- 优先使用 `scripts/gateway.sh` 调用接口。脚本从本文件头部的 `metadata.version` 读取版本并自动注入，调用处不要重复写死版本

```bash
bash "<skill-directory>/scripts/gateway.sh" \
  '{"api_name":"/store/search","keyword":"三体","count":10}'
```

### 请求 few-shot

**正确：传给调用脚本的业务参数平铺在 body 顶层；脚本会添加版本。**

```json
{"api_name":"/user/notebooks","count":100}
```
...
- 通过调用脚本发送 `{"api_name":"/_list"}` 可查看所有可用接口及参数定义

### 通用规则

1. **版本上报**：版本的唯一来源是本文件头部的 `metadata.version`。使用 `scripts/gateway.sh` 自动注入，不要省略版本或传 `"latest"`；这两种写法可能被服务端接受，但会绕过升级检测，并不表示使用最新版。如果回包中出现 `upgrade_info`，立即暂停当前接口操作并向用户说明；只有用户明确要求更新时才下载官方升级包，更新并重新读取技能后再执行原请求
